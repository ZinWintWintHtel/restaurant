...
<div class="bg-light m-5 text-lora">
        <h1 class="text-center text-warning py-3 ">Update Table Info</h1>
        <div class="p-5">
        <form class="forms-sample" action="{{route('staff.table_update')}}" method="post" >
            @csrf
            <input type="hidden" name="id" value="{{$table->id}}">
                    <div class="form-group row mb-3">
                      <label for="exampleInputUsername2" class="col-sm-3 col-form-label text-end">Table Name</label>
                      <div class="col-sm-9">
                        <input type="text" class="form-control @error('table_name') is-invalid @enderror" name="table_name" id="exampleInputUsername2" value="{{$table->table_name}}">
                        @error('table_name')
...
@enderror
                    ...
                    <div class="form-group row mb-3">
                      ...
                      <div class="col-sm-9">
                        <input type="number" class="form-control @error('max_capacity') is-invalid @enderror" name="max_capacity" id="exampleInputEmail2" value="{{$table->max_capacity}}">
                        @error('max_capacity')
    <p class="text text-danger">{{ $message }}</p>
@enderror
                      </div>
